...
Desain skema database (PostgreSQL menggunakan Prisma ORM) telah dipisahkan ke dalam dokumen modular khusus agar TRD tetap fokus pada Arsitektur Sistem dan API Contract. 

Silakan merujuk ke dokumen: [DATABASE.md](file:///docs/DATABASE.md) untuk melihat spesifikasi detail tabel `User` (termasuk kolom `role` USER/ADMIN dan saldo `tokens`), `Account`, `Session`, `VerificationToken`, serta log audit `Activity`.
